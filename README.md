...
| Foundry (AIServices) **+ project** | `spmm-rag-foundry` | **One resource for everything**: Content Understanding + `text-embedding-3-large` + `gpt-4o-mini` (CU figure captions) + **Vision multimodal embeddings** (image skill + query-time vectorizer). `allowProjectManagement` makes it visible in the new Foundry portal. |

All resources live in one **dedicated resource group** in a **single region** (default **West Europe**) that supports Content Understanding, the required models, *and* Vision multimodal embeddings together. (A single AIServices/Foundry account provides both CU and Vision multimodal, so no separate Vision resource is needed.)
...
- A subscription where you can create **Azure AI Search**, an **Azure AI Services (Foundry)** account, model deployments, and **role assignments** (you need `Owner` or `User Access Administrator` + `Contributor` to create the role assignment in the Bicep template).
- Model quota in your region for the text-embedding and chat-caption models. In **West Europe** use `text-embedding-3-large` + `gpt-4o-mini` on **`DataZoneStandard`** (plain `Standard` isn't offered for the embedding model there, and `gpt-4.1-mini` has no synchronous quota). Adjust `EMBED_CAPACITY` / `GPT_CAPACITY` in `.env` if your quota is lower.
...
**Resource names are auto-generated.** If you don't set `SEARCH_NAME` / `FOUNDRY_NAME`, step 1 generates globally-unique names as `<RESOURCE_PREFIX>-<role>-<random-suffix>` (default prefix `spmmrag`) and persists them to `.env.derived`, so re-runs reuse them. Set `RESOURCE_PREFIX` to brand them, or set the full names explicitly to pin them.

Optional overrides (defaults in `_common.ps1`): `RESOURCE_PREFIX`, `SEARCH_NAME`/`FOUNDRY_NAME`, `EMBED_*`, `CU_MODEL_*`, `GPT_*` (model deployments/SKUs/capacities), `SEARCH_SKU`, `INDEX_NAME`, `DATASOURCE_NAME`, `SKILLSET_NAME`, `INDEXER_NAME`, `SEARCH_API_VERSION`, and `QUERY_PRINCIPAL_ID` (force the Search Index Data Reader grant).

> If you pin names explicitly, the Search / Foundry names must be **globally unique**. If a name is taken, deployment preflight fails with `ServiceNameUnavailable` / `CustomDomainInUse` — pick a different name (or just leave them unset to auto-generate).
...
Creates the resource group, deploys `iac/main.bicep` (Search + Foundry account/project + models + role assignment), and captures endpoints into `.env.derived`.

- Grants the Search managed identity **Cognitive Services User** on the Foundry account (CU + embeddings + Vision multimodal).
...
- **Region:** everything is co-located in **one AIServices/Foundry account**, so the region must support Content Understanding, the models, **and** Vision multimodal embeddings together. **West Europe** and **Sweden Central** are validated; not all regions qualify.
